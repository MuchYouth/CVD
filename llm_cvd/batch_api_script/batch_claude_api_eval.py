"""Create and collect Anthropic Claude Message Batch API jobs for CVD evaluation."""
...
def parse_args() -> argparse.Namespace:
    ...
    prepare = subparsers.add_parser("prepare", help="Write Claude Message Batch input JSONL and manifest files.")
    add_common(prepare)
    add_dataset(prepare)
    ...
    submit = subparsers.add_parser("submit", help="Create a Claude Message Batch job from prepared JSONL.")
    add_common(submit)
    ...
    collect = subparsers.add_parser("collect", help="Download and parse an ended batch result stream.")
    add_common(collect)
...
def get_run_name(args: argparse.Namespace) -> str:
    ...
    if mode == "rag":
        return f"{args.db_name}_k{args.k}_claude_batch"
    return f"{args.db_name}_claude_batch"
...
def get_paths(args: argparse.Namespace) -> dict[str, Path]:
    ...
    return {
        ...
        "input_jsonl": output_dir / f"batch_claude_{run_name}_requests.jsonl",
        "manifest_jsonl": output_dir / f"batch_claude_{run_name}_manifest.jsonl",
        "job_json": output_dir / f"batch_claude_{run_name}_job.json",
        "raw_result_jsonl": output_dir / f"batch_claude_{run_name}_raw_results.jsonl",
        "result_jsonl": output_dir / f"{run_name}.jsonl",
        "result_csv": output_dir / f"{run_name}.csv",
        "parse_fails_json": output_dir / f"batch_claude_{run_name}_parse_fails.json",
    }
...
def require_anthropic():
    try:
        ...
        raise ModuleNotFoundError(
            "Claude Message Batch API uses the anthropic package. Install it with: pip install anthropic"
        ) from exc
    return anthropic
...
def make_client():
    ...
    if not api_key:
        raise RuntimeError("Set ANTHROPIC_API_KEY before using the Claude Message Batch API.")
...
def submit_batch(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with timed_step(f"Read {input_jsonl}"):
        requests = read_jsonl(input_jsonl)

    with timed_step("Create Claude message batch job"):
        batch_job = client.messages.batches.create(requests=requests)

    job_info = {
        "id": batch_job.id,
        "name": batch_job.id,
        "processing_status": batch_job.processing_status,
        "request_counts": object_to_dict(batch_job.request_counts),
        "model": args.model,
    ...
    log_step(f"Current state: {batch_job.processing_status}")
...
def get_batch_job(args: argparse.Namespace):
    ...
    return client.messages.batches.retrieve(job_name)
...
def status_batch(args: argparse.Namespace):
    ...
    log_step(f"Current state: {batch_job.processing_status}")
    log_step(f"Request counts: {json.dumps(object_to_dict(batch_job.request_counts), ensure_ascii=False)}")
    if getattr(batch_job, "results_url", None):
        log_step(f"Results URL: {batch_job.results_url}")
    return batch_job
...
def extract_text_from_response(result: dict[str, Any]) -> str | None:
    ...
    if result_body.get("type") != "succeeded":
        return None
    message = result_body.get("message") or {}
    text_parts = []
...
def make_result_record(meta: dict[str, Any], result: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any] | None]:
    ...
    if result_type != "succeeded":
        error = json.dumps(result_body, ensure_ascii=False)
...
def write_batch_results(client: Any, batch_id: str, path: Path) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", encoding="utf-8") as handle:
        for result in client.messages.batches.results(batch_id):
            handle.write(json.dumps(object_to_dict(result), ensure_ascii=False) + "\n")
...
def collect_batch(args: argparse.Namespace) -> None:
    ...
    if batch_job.processing_status != "ended":
        raise RuntimeError(f"Batch job is not complete: {batch_job.processing_status}")

    redownload = bool(getattr(args, "redownload", False))
    if redownload or not result_jsonl.exists():
        with timed_step(f"Download result stream for {batch_job.id}"):
            write_batch_results(client, batch_job.id, result_jsonl)
...
def run_all(args: argparse.Namespace) -> None:
    ...
    while True:
        ...
        state = batch_job.processing_status
...
def main() -> None:
    ...
    if args.model is None:
        args.model = os.getenv("CLAUDE_MODEL", "claude-sonnet-4-5-20250929")
